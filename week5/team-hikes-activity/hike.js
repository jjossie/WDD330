...
export default class Hikes {
  ...
  showOneHike(hikeName) {
    let listItem = renderOneHikeLight(this.getHikeByName(hikeName)); 
    this.parentElement.appendChild(listItem);
  }
  // in order to show the details of a hike ontouchend we will need to attach a listener AFTER the list of hikes has been built. The function below does that.
  addHikeListener() {
    // We need to loop through the children of our list and attach a listener to each, remember though that children is a nodeList...not an array. So in order to use something like a forEach we need to convert it to an array.
    let hikeItems = [...this.parentElement.children]
    for (let i = 0; i < hikeList.length; i++){
      hikeItems[i].addEventListener('click', () => {
        this.parentElement.appendChild(renderOneHikeFull(hikeList[i]));
      })
    }
